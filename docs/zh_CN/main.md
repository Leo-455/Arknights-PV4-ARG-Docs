...
接下来我们就可以在对照表中查找数据对应的值了。由于这张表格有整整8页，直接查找起来较为困难，在[使用程序解码](#使用程序解码)章节会介绍使用程序直接查找的方法，这里先介绍直接查找的方式。  
...
`'O', 'S2', 'V', 'S2', 'E', 'S2', 'R', 'S2', 'C', 'S2', 'O', 'S2', 'N', 'S2', 'T', 'S2', 'A', 'S2', 'C', 'S2', 'T', 'Sp', 'S2', 'B', 'S2', 'I', 'S2', 'N', 'S2', 'A', 'S2', 'R', 'S2', 'Y', '<NS>', '<NS>', '<NS>'`
...
基于以上原因，我在 FUTURE 攻坚组程序的基础上继续开发了[Leo-455/code49-decoder](https://github.com/Leo-455/code49-decoder)，加入了对模式5的支持和校验词检查。运行程序后，将会按照上文手动解码的步骤输出过程和结果：
...
> [!warning]
> 需要注意的是这个程序还目前还很不完善，仅支持字母编码，因此仅部分支持模式0、4、5，其它模式均不支持。
...
仔细观察这张图片，图中似乎是PV4中的一段。而Thank you for your support这段文字下面隐约能看到一些符号，处理后可以看出，是我们前面提到过的盲文，按照前文的方法可以解出文字“bymyside”，与一阶段中的”standbyherside”相呼应。这样的处理在PV3也同样出现过。
